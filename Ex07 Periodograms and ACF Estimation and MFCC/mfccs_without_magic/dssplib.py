...
def stft(x, fft_length=1024, frame_shift=128, #frame_shift is the hop size -chatGPT
         window_length=512, window='boxcar'):
    """
        # frequency bins = (framesize/2) + 1
        # nframes = (samples - framesize)/hopsize + 1. (valerio: STFT explained easily:21:42)
        # DFT has mirror symmetry around nyquist frequency, we can use only first real half.

        Example: 
        Signal = 10K samples
        Frame size = 1000 #frame size is the fft_length
        Hop size = 500    #hop size is the frame_shift

        #num_frequency bins = 1000/2+1 = 501 ->(0, sampling rate/2)
        #num_frames = (10000 - 1000)/500 + 1 = 19

        STFT -> (501, 19)
        
        Number of frames = (32000 - 400) / 160 + 1 = 199.375

        *******Time-Frequency tradeoffs*********
        frame_size up -> frequency res up, time res down
        frame_size down -> frequency res down, time res up

        ***Windowing is done to avoid discontinuities 
    """
...
def istft(X, window_length=512, frame_shift=128,
    ...
    nbins, nframes = X.shape	#nframes = (samples - framesize)/hopsize + 1. (valerio: STFT explained easily:21:42)
    if nbins == fft_length//2 +1: #DFT has mirror symmetry around nyquist frequency, we can use only first real half.
        ifft = np.fft.irfft
...
def mel_filterbank(nfilters=9, fft_length=1024, sample_rate=16000,
    ...
    """
        Mel = (#bands, framesize/2+1)
        Signal = (framesize/2+1, #frames)
        
        Useage:
        X_mel = filter_banks @ np.abs(X[:fft_length//2+1])**2/fft_length
        X_mel returns (#bands, #frames)
    """
    ...
    #creating triangular filter_banks
...
def mfcc(x, sample_rate=16000,
    ...
    """
        C(x(t)) = F^-1[log(F[x(t)])] : Cepstrum[logspectrum(spectrum[Time-domain signal])]
    """
    # STFT
    X = stft(x, frame_shift=frame_shift, fft_length=fft_length,
             window_length=window_length, window=window)
    # periodogram / estimated PSD, not true PSD because we applied windoings during STFT.
    ...
    # to cepstrum
    X_cep = sp_fft.dct(X_log_mel, n=nfilters, #discrete cosine transform: because we want real values, DFT or other transforms may give complex coefficients.
