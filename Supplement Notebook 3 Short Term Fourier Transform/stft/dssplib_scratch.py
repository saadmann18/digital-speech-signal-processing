...
# In[1]:
sample_rate, x = wav.read("fg_nt_upb_16k.wav") 
#x = x.astype(np.float)/np.abs(x).max()
...
num_zeros = int(np.ceil((len(x)- window_length)/frame_shift)*frame_shift + window_length - len(x))
x = np.concatenate((x, np.zeros(num_zeros)))

# preallocate the result matrix
num_frames = (len(x) - window_length)//frame_shift #number of frames created after multiplied the sampled signal by window function
X = np.zeros((fft_length, num_frames), dtype=np.complex128)

# processing
for m in range(num_frames):
    X[:, m] = np.fft.fft(x[m*frame_shift:m*frame_shift+window_length]*w, fft_length)
X
# In[2]:
...
for m in range(nframes):
    X_transform = X[:fft_length//2 +1, m].copy()  #fft_length=1024 temporarily holding values for all X.
    #taken as half because complex DFT is divided into positive and negative frequencies with nyquist in the center.
    x_n = np.fft.irfft(X_transform, axis=0)
    x_out[m*frame_shift:m*frame_shift + window_length] += w* x_n[:window_length].real
