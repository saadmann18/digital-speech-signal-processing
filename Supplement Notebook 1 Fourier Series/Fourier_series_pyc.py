...
T = 3 #period
L = 1.5 #?
t = np.linspace(-L*T, L*T, 6000)  #t axis spacing
...
def plot_fourier_series(a_k, x_ref, t, T):
    ...
    plt.axhline(c='C7')
    plt.plot(t / T, x_ref, alpha=0.5, lw=3)
    plt.plot(t / T, x_c, 'r', lw=2);
    plt.axvline(c='C7')
    plt.grid(True);
...
# Funktion, um Fouriersumme zusammen mit wiederholtem Impuls zu plotten
